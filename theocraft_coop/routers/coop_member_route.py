...
async def get_coop_membership(member_id: UUID, cooperative_id: UUID, current_user_profile: Current_User):
...
async def update_membership(member_update: schemas.MembershipUpdate, current_user_profile: Current_User):
