...
import coop_connect.database.db_handlers.user_db_handler as user_db_handler
import coop_connect.root.dependencies as dep
import coop_connect.schemas.user_schemas as schemas
import coop_connect.services.service_utils.auth_utils as auth_utils
import coop_connect.services.service_utils.token_utils as toks_utils
from coop_connect.root.connect_exception import (
    ConnectAuthException,
    ConnectBadRequestException,
    ConnectNotFoundException,
)
from coop_connect.root.utils.ebulk_sms import send_sms
from coop_connect.root.utils.send_mail import send_mail
...
async def get_user(id: UUID):
    try:
        ...
        raise ConnectNotFoundException(message="user not found")
...
async def user_mfa_sign_up(phone_number: Optional[schemas.PhoneNumber] = None, email: Optional[schemas.EmailStr] = None):  # type: ignore
    ...
    try:
        ...
        raise ConnectBadRequestException(message="phone_number or email exist for user")
...
async def verify_mfa_token(code: str):
    ...
    if datetime.now() > mfa_token.code_expires_at:
        ...
        raise ConnectBadRequestException(message="verification code has expired")
...
async def user_update(user_update: schemas.UserUpdate, user_id: UUID):
    try:
        ...
        raise ConnectBadRequestException(message="user update failed")
